...
public class AlxLocationManager implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    public static final String GOOGLE_API_KEY = "AIzaSyAzXDppoO9uFp-AzNhP4D6Zq2KH8LmdVKA";
...
//            Toast.makeText(context,"恭喜你安装了谷歌框架，发给google api 的json是；："+json,Toast.LENGTH_LONG).show();
    ...
    public boolean getCurrentLocation() {
        if (ActivityCompat.checkSelfPermission(MyApplication.mContext, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED &&
                ActivityCompat.checkSelfPermission(MyApplication.mContext, Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            // TODO: Consider calling
            //    ActivityCompat#requestPermissions
            // here to request the missing permissions, and then overriding
            //   public void onRequestPermissionsResult(int requestCode, String[] permissions,
...
//            if(isDebugging)Toast.makeText(context,"最终记录的经过火星转换的结果是latitude="+latitude+"   longitude="+longitude+"   accuracy="+accuracy,Toast.LENGTH_LONG).show();
    ...
    public static GeoLocationAPI getCellInfo(Context context) {
        ...
        ArrayList<GoogleCellTower> towers = new ArrayList<GoogleCellTower>(1);
        ...
        if (Build.VERSION.SDK_INT < 17) {//低版的android系统使用getNeighboringCellInfo方法
            ...
            towers = new ArrayList<GoogleCellTower>(infos.size());
            ...
            if (infos != null) {
                ...
                towers = new ArrayList<GoogleCellTower>(infos2.size());
    ...
    public static GeoLocationAPI getWifiInfo(Context context, GeoLocationAPI geoLocationAPI) {
        ...
        ArrayList<GoogleWifiInfo> wifiInfos = new ArrayList<GoogleWifiInfo>(lsAllWIFI.size());
