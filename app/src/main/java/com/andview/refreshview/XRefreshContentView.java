...
//        mTotalItemCount = 0;
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (mSilenceLoadMore) {
            ...
            DebugLog.i("wang","===mPreLoadCount=="+mPreLoadCount);
            if (mPreLoadCount == 0) {
                if (isBottom()) {
                    if (!mIsLoadingMore) {
    ...
    public boolean canChildPullUp() {
        if (child instanceof AbsListView) {
            AbsListView absListView = (AbsListView) child;
          //  DebugLog.i("wang","=====can=="+ canScrollVertically(child, 1)+"==last=="+absListView.getLastVisiblePosition()+"===t==="+(mTotalItemCount - 1));
            return canScrollVertically(child, 1)
